...
void process_input(cv::Mat& src, float* input_device_buffer) {
  cuda_preprocess(
    src.ptr(), src.cols, src.rows,
    input_device_buffer, kInputW, kInputH);
}

inline cv::Mat image_affine(cv::Mat& src) {
...
void process_input_cv_affine(cv::Mat& src, float* input_device_buffer) {
  ...
  auto warp_dst = image_affine(src);

  cuda_pure_preprocess(
    warp_dst.ptr(), input_device_buffer, kInputW, kInputH);
}
